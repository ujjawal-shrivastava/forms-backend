...
def get_token_from_cookie(request):
    """Retrieves the http authorization header from the cookie"""
    token = None
...
def create_jwt(user, extra_payload={}):
    ...
    expiration_delta = getattr(
        settings, "JWT_EXPIRATION_DELTA", datetime.timedelta(minutes=60)
    )
...
def create_long_jwt(user, extra_payload={}):
    """Creates a Long Term JWT for an authenticated user"""
    if not user.is_authenticated:
        raise AuthenticatedUserRequiredError(
            "JWT generationr requires an authenticated user"
        )
    ...
    expiration_delta = getattr(
        settings, "JWT_REFRESH_TOKEN_EXPIRATION_DELTA", datetime.timedelta(days=7)
    )
